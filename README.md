...
O CI exige pelo menos 80% de cobertura de linhas em `GearUp.Domain`. Comandos
de cobertura, análise de vulnerabilidades (SonarCloud) e detalhes de cada
projeto de teste: [docs/Testes/Documentação de Testes.md](docs/Testes/Documentação%20de%20Testes.md).
...
A pasta `k8s/` tem dois ambientes independentes — `k8s/dev/` (Minikube,
Postgres local) e `k8s/prod/` (banco gerenciado via `ExternalSecret`) — com
Deployment, Service, ConfigMap/Secret e HPA para a API.

Passo a passo de deploy local (Minikube) e produção, tabela de manifestos por
ambiente e scripts de validação (`scripts/validate-k8s-*.sh`):
[docs/Kubernetes/Documentação Kubernetes.md](docs/Kubernetes/Documentação%20Kubernetes.md).

...
namespace GearUp.Infrastructure.Persistence.Repositories
{
    ...
    {
        public async Task AdicionarAsync(Estoque item, CancellationToken ct)
        {
            await db.EstoqueItens.AddAsync(item, ct);
        }

        public Task<Estoque?> ObterAsync(Guid id, CancellationToken ct)
        {
            return db.EstoqueItens.Include(x => x.Movimentacoes).SingleOrDefaultAsync(x => x.Id == id, ct);
        }

        public async Task<IReadOnlyList<Estoque>> ListarAsync(CancellationToken ct)
        {
